...
if (requestedConfigFile) {
    ...
    for (const configFile of configFilesToSearch) {
        try {
            // Auto-detected config files are always loaded from the bot's folder, even if the cwd is differed
...
if (configFileToLoad) {
    ...
    try {
        if (configFileToLoad.endsWith(".js")) {
            ...
            if (configFileToLoad.endsWith(".ini") || configFileToLoad.endsWith(".ini.text")) {
...
function exitWithConfigurationErrors(errors) {
    ...
    console.error("");
    console.error("Please restart the bot after fixing the issues mentions above.");
    console.errror("");
...
const thuthyValues = ["1", "true", "on", "yes"];
...
const configIsValid = valiedate(config);
if (! configIsValid) {
